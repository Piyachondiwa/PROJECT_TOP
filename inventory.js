...
function countItems(items) {
  return Object.values(items || {}).reduce((sum, count) => sum + Math.max(0, count || 0), 0);
}

function getFoodIds() {
  return Object.entries(player.foods || {})
...
function getSelectedFoodId() {
  ...
  if (!inventoryState.selectedFoodId || !foods.includes(inventoryState.selectedFoodId)) {
    inventoryState.selectedFoodId = foods[0];
  }
...
function eatSelectedFood() {
  ...
  // Temporary prototype implementation: eating grants the monster trait
  // and adds its skill to the available skill pool. The permanent loadout
  // system will be expanded later without changing inventory storage.
  player.activeTraits = player.activeTraits || {};
  player.activeTraits[foodId] = {
    name: monster.trait,
    description: monster.traitDescription,
  };
...
function cycleFoodSelection() {
  const foods = getFoodIds();
  if (!foods.length) {
    inventoryState.selectedFoodId = null;
    return;
  }
  const current = foods.indexOf(getSelectedFoodId());
  inventoryState.selectedFoodId = foods[(current + 1) % foods.length];
  renderInventory();
}
...
function renderInventory() {
  ...
  const foodHtml = foods.length
    ? foods.map((id) => {
        ...
        return `<button class="item-row food-item${selectedClass}" data-food-id="${id}" type="button"><span>${monster?.name || id}</span><b>x${player.foods[id]}</b></button>`;
      }).join('')
    : '<div class="empty">No Monster Food</div>';

  const traitHtml = traitIds.length
    ? traitIds.map((id) => `<div class="trait-card"><b>${MONSTER_DATA[id]?.trait || id}</b><span>${MONSTER_DATA[id]?.traitDescription || ''}</span></div>`).join('')
