...
const monsters = [
  { id: 'goblin', name: 'Goblin', x: 700, y: 380, w: 34, h: 38, hp: 45, maxHp: 45, level: 2, element: 'nature', seed: 'Goblin Seed', color: '#778c58', speed: 55, alive: true, hitFlash: 0 },
  { id: 'slime', name: 'Fire Slime', x: 830, y: 520, w: 38, h: 30, hp: 55, maxHp: 55, level: 3, element: 'fire', seed: 'Fire Slime Seed', color: '#a86d4b', speed: 38, alive: true, hitFlash: 0 },
  { id: 'wolf', name: 'Wolf', x: 560, y: 610, w: 44, h: 30, hp: 65, maxHp: 65, level: 4, element: 'nature', seed: 'Wolf Seed', color: '#77746f', speed: 75, alive: true, hitFlash: 0 },
];
...
function burst(x, y, color, count) {
  for (let i = 0; i < count; i++) particles.push({ x, y, vx: (Math.random()-.5)*100, vy: (Math.random()-.5)*100, life: .35 + Math.random()*.35, color });
}
...
function drawWorld() {
  ...
  // Simple prototype terrain. Replace with your tileset later.
...
function drawPlayer() {
  ctx.save(); ctx.translate(Math.round(player.x - camera.x), Math.round(player.y - camera.y));
  const bob = (keys.size && (keys.has('KeyW') || keys.has('KeyA') || keys.has('KeyS') || keys.has('KeyD'))) ? Math.sin(performance.now()/90)*2 : 0;
  ctx.translate(0, bob);
  // Placeholder character. Replace with supplied pixel sprite sheet.
  ...
  if (player.attackTimer > 0) {
    ctx.strokeStyle = '#d9d2b5'; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(0, 3, 45, -1.2, 1.2); ctx.stroke();
  }
  if (player.dodgeTimer > 0) { ctx.globalAlpha = .45; ctx.fillStyle = '#d8d0b0'; ctx.fillRect(-22,-25,44,65); }
  ctx.restore();
}

function drawMonster(m) {
  ctx.save(); ctx.translate(Math.round(m.x-camera.x), Math.round(m.y-camera.y));
  ctx.fillStyle = 'rgba(0,0,0,.25)'; ctx.fillRect(-m.w/2, m.h/2, m.w, 6);
  ctx.fillStyle = m.hitFlash > 0 ? '#f2d9c5' : m.color;
  if (m.id === 'slime') { ctx.beginPath(); ctx.arc(0, 0, 18, Math.PI, 0); ctx.lineTo(18, 13); ctx.lineTo(-18, 13); ctx.closePath(); ctx.fill(); }
  else if (m.id === 'wolf') { ctx.beginPath(); ctx.moveTo(-23,10); ctx.lineTo(-8,-10); ctx.lineTo(10,-12); ctx.lineTo(24,4); ctx.lineTo(12,14); ctx.lineTo(-15,14); ctx.closePath(); ctx.fill(); }
  else { ctx.fillRect(-15,-17,30,34); ctx.fillRect(-19,-21,8,8); ctx.fillRect(11,-21,8,8); }
...
function drawParticle(p) {
  ctx.save(); ctx.globalAlpha = Math.max(0, Math.min(1, p.life*2)); ctx.fillStyle = p.color; ctx.fillRect(Math.round(p.x-camera.x), Math.round(p.y-camera.y), 4, 4); ctx.restore();
}

function drawNightOverlay() {
  const night = worldTime >= 19 || worldTime < 6;
  if (!night) return;
  const darkness = worldTime >= 21 || worldTime < 4 ? .42 : .24;
